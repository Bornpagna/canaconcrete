...
class Sales_Model_DbTable_DbDeliverys extends Zend_Db_Table_Abstract
{
	...
	function getItemOrder($id){
		$db = $this->getAdapter();
		$sql = "SELECT so.`qty_order`,so.`pro_id`,s.`branch_id` FROM `tb_sales_order` AS s,`tb_salesorder_item` AS so WHERE s.id=so.`saleorder_id` AND so.`saleorder_id`=$id";
		return $db->fetchAll($sql);
	}
